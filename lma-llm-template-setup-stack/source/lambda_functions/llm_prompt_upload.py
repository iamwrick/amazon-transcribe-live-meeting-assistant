...
def lambda_handler(event, context):
    ...
    try:
        if the_event in ('Create', 'Update'):
            ...
            # ===== WT Added for Healthcare domain ====
            domain = os.environ.get('DOMAIN', '').lower()  # Get the "Domain" value from the environment variable

            # Load the appropriate template.json file based on the "Domain" value
            if domain.lower() == 'healthcare':
                llm_prompt_summary_template_file = os.environ['LAMBDA_TASK_ROOT'] + "/LLMPromptHealthcareSummaryTemplate.json"
            else:
